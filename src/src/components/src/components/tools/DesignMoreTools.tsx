    ...
    sizes.forEach(size => {
      ...
      if (ctx) {
        ...
        img.onload = () => {
          ...
          if (Object.keys(results).length === sizes.length) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 rounded-xl border border-slate-800 bg-slate-950 space-y-4">
          ...
          {image && (
            ...
            >
              ⚙️ {language === 'en' ? 'Generate Multi-Size Favicons' : 'توليد المقاسات المتعددة'}
            </button>
          )}
        </div>

        <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/40 min-h-[160px] flex flex-col justify-center">
