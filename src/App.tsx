...
export default function App() {
  const getInitialRoute = (): { tab: any; toolId: string | null } => {
    ...
    if (['about', 'privacy', 'terms', 'contact', 'tools', 'favorites', 'history', 'settings'].includes(cleanPath)) {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'tools' | 'favorites' | 'history' | 'settings' | 'about' | 'privacy' | 'terms' | 'contact' | '404'>(initialRoute.tab);
  ...
  const navigateTo = (tab: 'home' | 'tools' | 'favorites' | 'history' | 'settings' | 'about' | 'privacy' | 'terms' | 'contact' | '404', toolId: string | null = null) => {
    let path = '/';
    if (toolId) {
      path = `/tool/${toolId}`;
    } else if (tab !== 'home') {
  ...
  const [gaMeasurementId, setGaMeasurementId] = useState<string>(() => {
    return localStorage.getItem('toolix_ga_measurement_id') || (import.meta as any).env.VITE_GA_MEASUREMENT_ID || '';
  });
  ...
  useEffect(() => {
    ...
    const currentUrl = `https://toolix.app${selectedToolId ? `/tool/${selectedToolId}` : activeTab === 'home' ? '' : `/${activeTab}`}`;
    ...
    if (selectedToolId && activeTool) {
      ...
      schemaObj = {
        ...
        "@type": "WebSite",
        "name": "Toolix",
        "url": "https://toolix.app",
        "description": description,
        "potentialAction": {
          "@type": "SearchAction",
          "target": "https://toolix.app/?search={search_term_string}",
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-lg transition-colors duration-300 ${darkMode ? 'bg-[#030712]/80 border-slate-900' : 'bg-white/80 border-slate-200'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3.5 cursor-pointer" onClick={() => navigateTo('home')}>
            <img 
              src="/logo.png" 
              className="w-20 h-20 rounded-2xl object-contain shadow-xl shadow-blue-500/10 border border-slate-800/10 dark:border-slate-800/40" 
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-28 relative z-10">
        ...
        {isLaunchingTool ? (
          ...
          <div className="space-y-6">
            ...
            {activeTab === '404' && (
              <div className="max-w-md mx-auto py-16 text-center space-y-6 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                <div className="flex justify-center">
                  <div className={`p-5 rounded-full ${darkMode ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-600'}`}>
                    <AlertTriangle className="w-12 h-12" />
                  </div>
                </div>
                <h2 className={`text-2xl font-black font-display tracking-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                  {language === 'en' ? '404 - Tool Not Found' : '404 - الأداة غير موجودة'}
                </h2>
                <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                  {language === 'en' 
                    ? 'The requested digital utility tool does not exist on our platform or has been relocated. Please explore our 50+ other premium free utilities.' 
                    : 'الأداة الرقمية المطلوبة غير متوفرة على منصتنا حالياً. يرجى تصفح أكثر من 50 أداة مميزة ومجانية أخرى.'}
                </p>
                <div className="flex justify-center gap-4">
                  <button
                    onClick={() => navigateTo('home')}
                    className="px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 transition-all cursor-pointer"
                  >
                    {language === 'en' ? 'Go to Home' : 'الذهاب للرئيسية'}
                  </button>
                  <button
                    onClick={() => navigateTo('tools')}
                    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl border transition-all cursor-pointer ${darkMode ? 'border-slate-800 bg-slate-950 text-slate-300 hover:bg-slate-900' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}
                  >
                    {language === 'en' ? 'Browse All Tools' : 'تصفح كل الأدوات'}
                  </button>
                </div>
              </div>
            )}
